...
try {
    ...
    \IPS\Db::i()->insert('core_applications', [
    ...
    foreach ($areaModules as $key => $module) {
        try {
            \IPS\Db::i()->select(
                'sys_module_id',
                'core_modules',
                [ 'sys_module_application=? AND sys_module_area=? AND sys_module_key=?', $appDir, $area, $key ],
            )->first();
...
$orphans = iterator_to_array(\IPS\Db::i()->select(
    'id, filename',
    'core_hooks',
    [ 'app=? AND ' . \IPS\Db::i()->in('filename', $expectedFilenames, true), $appDir ],
));
foreach ($orphans as $o) {
    \IPS\Db::i()->delete('core_hooks', [ 'id=?', $o['id'] ]);
    echo "[-] hook {$o['filename']} removed (not in hooks.json)\n";
}

foreach ($hooks as $filename => $hook) {
    try {
        \IPS\Db::i()->select('id', 'core_hooks', [ 'filename=? AND app=?', $filename, $appDir ])->first();
        echo "[=] hook $filename\n";
    } catch (UnderflowException $e) {
        \IPS\Db::i()->insert('core_hooks', [
            'type' => $hook['type'],
            'class' => $hook['class'],
            'filename' => $filename,
            'app' => $appDir,
...
if (file_exists("$appPath/data/tasks.json")) {
    ...
    foreach ($tasks as $taskKey => $taskVal) {
        ...
        try {
            \IPS\Db::i()->select('id', 'core_tasks', [ '`key`=? AND app=?', $taskKey, $appDir ])->first();
            echo "[=] task $taskKey\n";
        } catch (UnderflowException $e) {
...
if (file_exists("$appPath/data/widgets.json")) {
    ...
    foreach ($widgets as $widgetKey => $widget) {
        try {
            \IPS\Db::i()->select('id', 'core_widgets', [ '`key`=? AND app=?', $widgetKey, $appDir ])->first();
            echo "[=] widget $widgetKey\n";
        } catch (UnderflowException $e) {
            \IPS\Db::i()->insert('core_widgets', [
                'app' => $appDir,
                'key' => $widgetKey,
                'class' => $widget['class'] ?? '',
                'restrict' => json_encode($widget['restrict'] ?? []),
 ...
 */
$jslang = [];
if (file_exists("$appPath/dev/jslang.php")) {
    include "$appPath/dev/jslang.php";
}

$langCache = [];
$loadLang = function ($code) use (&$langCache, $appPath) {
    $code = strtolower($code);
    if (isset($langCache[ $code ])) {
...
foreach ($installedLangs as $row) {
    ...
    foreach ([ [ $lang, 0 ], [ $jslang, 1 ] ] as $pair) {
        [$map, $isJs] = $pair;
        foreach ($map as $key => $val) {
            try {
                $existing = \IPS\Db::i()->select(
                    'word_id, word_default, word_custom',
                    'core_sys_lang_words',
                    [ 'word_app=? AND word_key=? AND lang_id=? AND word_js=?', $appDir, $key, $langId, $isJs ],
                )->first();
                ...
                \IPS\Db::i()->update('core_sys_lang_words', $update, [ 'word_id=?', $existing['word_id'] ]);
                $langUpdated++;
            } catch (UnderflowException $e) {
...
if (is_dir($htmlPath)) {
    ...
    foreach ($it as $file) {
        ...
        try {
            $tplId = \IPS\Db::i()->select('template_id', 'core_theme_templates', [
...
if (is_dir($cssPath)) {
    ...
    \IPS\Db::i()->update('core_themes', [ 'set_css_updated' => time() ]);
}
echo "[+] CSS imported: inserted=$cssInserted, updated=$cssUpdated\n";

/* 9. clear caches, purge datastore, compile templates */
\IPS\Db::i()->delete('core_store', [ 'store_key LIKE ?', 'template_compiling_%' ]);
...
\IPS\Db::i()->delete('core_store', [ 'store_key LIKE ?', 'css_compiling_%' ]);
...
$groups = iterator_to_array(\IPS\Db::i()->select(
    'DISTINCT template_location, template_group',
    'core_theme_templates',
    [ 'template_app=?', $appDir ],
));
$cssFiles = iterator_to_array(\IPS\Db::i()->select(
    'DISTINCT css_location, css_path, css_name',
    'core_theme_css',
    [ 'css_app=?', $appDir ],
));
